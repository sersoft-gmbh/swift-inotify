...
#else if __APPLE__
#warning Unsupported platform! The package will build but not work!

/*
 We mock stuff out here, so that we can build on macOS. All non-functional however!
 */
...
/* Create and initialize inotify instance.  */
static int inotify_init1(int __flags) {
    return 0;
}

/* Add watch of object NAME to inotify instance FD.  Notify about
   events specified by MASK.  */
static int inotify_add_watch(int __fd, const char *__name, uint32_t __mask) {
    return 0;
}

/* Remove the watch specified by WD from the inotify instance FD.  */
static int inotify_rm_watch(int __fd, int __wd) {
    return 0;
}
#endif
